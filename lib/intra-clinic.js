...
exports["enter_intra_clinic_post"] = function(conn, req, res, cb){
	...
	conn.query("insert into post set content = ?, created_at = date(now())", [content], function(err, result){
		if( err ){
			cb(err);
			return;
		}
...
exports["enter_intra_clinic_comment"] = function(conn, req, res, cb){
	...
	conn.query("insert into comment set name = ?, content = ?, post_id = ?, created_at = date(now())",
		[name, content, postId], function(err, result){
		if( err ){
			cb(err);
			return;
		}
