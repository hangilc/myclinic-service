...
function addTagToPost(conn, tagId, postId, done){
	conn.query("insert into tag_post set tag_id = ?, post_id = ? ",
		[tagId, postId], done);
}

function removeTagFromPost(conn, tagId, postId, done){
	conn.query("delete from tag_post where tag_id = ? and post_id = ?", 
		[tagId, postId], function(err, result){
		if( err ){
			done(err);
			return;
		}
		if( result.affectedRows === 1 ){
			...
			done("remove post from tag failed: " + result.affectedRows);
		}
	})
}

exports["add_intra_clinic_tag_to_post"] = function(conn, req, res, cb){
	...
	addTagToPost(conn, tag_id, post_id, function(err){
		if( err ){
			cb(err);
			return;
		}
...
exports["remove_intra_clinic_tag_from_post"] = function(conn, req, res, cb){
	var tag_id = +req.body.tag_id;
	var post_id = +req.body.post_id;
	if( !(tag_id > 0) ){
		cb("invalid tag_id");
	...
	removeTagFromPost(conn, tag_id, post_id, function(err){
		if( err ){
			cb(err);
			return;
		}
...
exports["batch_modify_intra_clinic_tags_for_post"] = function(conn, req, res, cb){
	var postId = +req.body.post_id;
	...
	conti.forEach(commands, function(cmd, done){
		var action = cmd.action;
		var tagId = cmd.tag_id;
		if( !(tagId > 0) ){
			done("invalid tag_id");
			return;
		}
		if( action === "add" ){
			addTagToPost(conn, tagId, postId, done);
		} else if( action === "remove" ){
			removeTagFromPost(conn, tagId, postId, done);
		} else {
