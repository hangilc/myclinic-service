...
reg("get_full_visit", function(conn, req, res, cb){
	var visitId = +req.query.visit_id;
	db.getFullVisit(conn, visitId, cb);
});
...
reg("get_gazou_label", function(conn, req, res, cb){
	var conductId = +req.query.conduct_id;
	db.getGazouLabel(conn, conductId, cb);
});
...
reg("get_conduct_shinryou", function(conn, req, res, cb){
	var conductShinryouId = +req.query.conduct_shinryou_id;
	db.getConductShinryou(conn, conductShinryouId, cb);
})
...
reg("get_conduct_drug", function(conn, req, res, cb){
	var conductDrugId = +req.query.conduct_drug_id;
	db.getConductDrug(conn, conductDrugId, cb);
})
...
reg("get_conduct_kizai", function(conn, req, res, cb){
	var conductKizaiId = +req.query.conduct_kizai_id;
	db.getConductKizai(conn, conductKizaiId, cb);
})
...
reg("search_pharma_drug", function(conn, req, res, cb){
	var text = req.query.text;
	var sql = "select p.iyakuhincode as code from pharma_drug p, iyakuhin_master_arch m " +
		" where p.iyakuhincode = m.iyakuhincode and m.name like ? " +
		" group by p.iyakuhincode ";
	conn.query(sql, ["%" + text + "%"], function(err, result){
		if( err ){
			cb(err);
			return;
		}
		cb(undefined, result.map(function(r){ return r.code; }));
	});
});

reg("search_most_recent_iyakuhin_master", function(conn, req, res, cb){
	var text = req.query.text;
	var sql = "select m.iyakuhincode as code from iyakuhin_master_arch m " +
		" where m.name like ? " +
		" group by m.iyakuhincode ";
	conn.query(sql, ["%" + text + "%"], function(err, result){
		if( err ){
			cb(err);
			return;
...
reg("batch_get_most_recent_iyakuhin_master", function(conn, req, res, cb){
	var iyakuhincodes = req.body.iyakuhincodes;
	var masters = [];
	conti.forEach(iyakuhincodes, function(iyakuhincode, cb){
		var sql = "select * from iyakuhin_master_arch m " + 
			" where m.iyakuhincode = ? order by valid_from desc limit 1 ";
		conn.query(sql, [iyakuhincode], function(err, result){
			if( err ){
				cb(err);
				return;
			}
			masters.push(result[0]);
			cb();
		})
	}, function(err){
		if( err ){
		...
		cb(undefined, masters);
	});
});

reg("enter_pharma_drug", function(conn, req, res, cb){
	var iyakuhincode = req.body.iyakuhincode;
	var description = req.body.description;
	var sideeffect = req.body.sideeffect;
	db.insertPharmaDrug(conn, {
...
reg("update_pharma_drug", function(conn, req, res, cb){
	var iyakuhincode = req.body.iyakuhincode;
	var description = req.body.description;
	var sideeffect = req.body.sideeffect;
	var sql = "update pharma_drug set description = ?, " +
		" sideeffect = ? where iyakuhincode = ? ";
	conn.query(sql, [description, sideeffect, iyakuhincode], function(err, result){
		if( err ){
			cb(err);
			return;
		}
		if( result.affectedRows === 1 ){
			...
			cb("update pharma_drug failed: " + result.affectedRows);
		}
	})	
});

reg("get_pharma_drug", function(conn, req, res, cb){
	var iyakuhincode = +req.query.iyakuhincode;
	db.getPharmaDrug(conn, iyakuhincode, cb);
});

reg("delete_pharma_drug", function(conn, req, res, cb){
	var iyakuhincode = req.body.iyakuhincode;
	var sql = "delete from pharma_drug where iyakuhincode = ?";
	conn.query(sql, [iyakuhincode], function(err, result){
		if( err ){
			cb(err);
			return;
		...
		if( result.affectedRows === 1 ){
			...
			cb("delete pharma_drug failed: " + result.affectedRows);
		}
	})
})
